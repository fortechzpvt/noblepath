...
- FR-5.7 For a single trip, the pickup and drop-off can be searched, pinned on a map (tap or drag), and the pickup set from the visitor's current location with their consent. Pins are optional; the typed place is always accepted (D-25).
...
| FR-5 Bookings | `docs/design/user-flows.md` | `app/bookings/`, `app/api/bookings/`, `app/api/rides/`, `app/api/places/` | `docs/security/security-review.md` |
