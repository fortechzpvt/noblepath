...
### F10 — Book a single trip (D-24, map and search D-25)
A point-to-point journey with a driver (e.g. Mannar → Jaffna), requested on its own without a full trip. Shown to travellers as **A single trip** (it was *A single ride* in D-24; code, API `/api/rides` and `?service=ride` keep the "ride" name). Built; there is no payment and no confirmed booking — like the trip request (D-23) it produces a **request ID** and staff reply with a quotation by email.

1. `/bookings` opens with a radio-card switch, *"What would you like to book?"*: **A full trip** (default) · **A single trip**. `/bookings?service=ride` opens with **A single trip** selected. Choosing a card mirrors the choice into `?service=` with `replaceState` (no extra history entry). Both forms stay mounted and the inactive one is `hidden`, so switching back and forth keeps everything typed in either form.
2. **Set pickup and drop-off** — any of these, in any mix; the map is optional and every step can be done without it:
   - **Search** — Pickup location / Drop-off location are comboboxes. Typing shows matching known towns and airports at once; after a 300 ms pause in typing (2+ characters) results from `/api/places/search` are merged in (max 8). ↓/↑ move the highlight (focus stays in the input), **Enter** picks the highlighted option (with nothing highlighted, Enter submits the form as normal), **Escape** closes the list; a pointer press picks an option; blurring closes the list after 150 ms. Choosing a result fills the field with its label and **pins** it (*"Pinned on the map (lat, lng) · Remove pin"*); choosing the pickup moves the map to *Setting drop-off*. Typing in a field clears its pin. **Remove pin** removes only the pin; the text stays. If search is down the field still takes free text (*"Search is unavailable right now. Type the place, or tap the map to pin it."*). Free text without a pin is accepted.
   - **Use my current location** (pickup only) — asks the browser for location only on this press. Live messages: *"Finding your location…"*; success *"Pickup set to your current location, accurate to about N m. Drag pin A on the map if it is not quite right."* (field text *"My current location"*); or permission denied / timeout / unavailable / outside Sri Lanka, each pointing back to search or the map.
   - **Map** — fieldset *"Pin it on the map"* (optional). Radio chips **Setting pickup** (A) / **Setting drop-off** (B) choose which pin a tap places; focusing a place field also switches the chip to that end. Tapping the map (inside Sri Lanka only) places the active pin; either pin can be dragged. A pin placed without a name (tap, drag, current location) sets the field to *"Pinned location (lat, lng)"* at once, then to *"Near <nearest named place>"* from `/api/places/reverse` if the traveller has not typed over it. The map refits to both pins (or centres on one) when a pin arrives from search or location, not when moved on the map. A dashed line joins A and B.
   - **Swap pickup and drop-off** swaps text and pins (shown once either field has text).
   - With both ends pinned, an estimate box appears: *"About N km by road, roughly H h M min. This is an estimate from the straight-line distance; your quotation confirms the route and price."* (straight line × 1.3 at 40 km/h; no routing service).
3. **Fill in the rest** (same page): Trip type chips (**One way** default · **Return**), Pickup date (min = today) and Pickup time; **Return** reveals Return date (min = outward date) and Return pickup time (drop-off back to pickup). *Vehicle and passengers* — the shared vehicle radio grid, Passengers (1 – max travellers, default 2), Pieces of luggage (0–50, default 2), Notes for the driver (optional, 1000 chars). *Your details* — Full name, Email, WhatsApp or phone number (with country code). **Review my trip** primary button, with *"You will see a full summary before anything is submitted."*
4. **Review** — H2 *"Check your trip"*, a read-only summary (Your trip — From, Pickup pin, To, Drop-off pin, Estimate when both pinned, Outward, Return… · Contact · Price = "Quotation"), Terms and conditions (now mentioning map pins and shared location) with a required, unticked checkbox, **Submit trip request** (primary) and **Edit my trip** (outline, returns to the form with values and pins intact).
5. **Done** — card *"Your trip request"* with *"Trip request ID"* in monospace, a **Copy ID** button, and a line naming the email address the reply goes to. The staff email (*"Single trip request …"*) carries each pin with a map link, a directions link and the rough estimate, or a note to confirm addresses when both ends are not pinned.

| State | UI |
|---|---|
| **Default** | Form step, one way selected, passengers 2, luggage 2, no vehicle, no pins, *Setting pickup* chip selected, map centred on Sri Lanka |
| **Searching** | Place field status *"Searching…"*, then *"N places found. Use the arrow keys to choose."* or *"No places found. Try another spelling, or tap the map."* (polite live region) |
| **Map unavailable** | Map replaced by *"The map could not load. Search for your pickup and drop-off above instead."*; search and free text still work |
| **Error (form-level on review)** | Client validation on **Review my trip** only (no blur validation). Error summary at the top of the form, `role="alert"`, focused, with anchor links to each field (`rd-pickup`, `rd-dropoff`, …); per-field messages below each field. Rules: both places ≥ 2 chars and not the same place (case/space-insensitive, or pins under 100 m apart); any pin inside Sri Lanka; date today or later; valid times; for Return, return date on/after the outward date and, on the same day, a later time; a vehicle chosen; whole-number passengers and luggage in range; name, email and phone format. Server pin errors are pointed at the matching place field |
| **Error (terms)** | Submitting without the checkbox shows the error summary with a link to the checkbox |
| **Loading (submitting)** | **Submit trip request** is `disabled` while the request is in flight (no spinner, no `aria-busy`) |
...
| **Success** | Done step, as above. A one-way trip never sends return values to the server |
