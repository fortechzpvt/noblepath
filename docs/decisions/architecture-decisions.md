...
- Verified with `npm run lint`, `npm run typecheck`, `npm run build` (the last with placeholder `RESEND_API_KEY`/`BOOKINGS_NOTIFICATION_EMAIL` supplied inline, never committed — see previous bullet), and manual `curl` checks of every response path (`200` honest success, `200` honeypot-faked success, `400` malformed JSON, `400` validation failure, `413` oversized body, `415` wrong content type, `429` rate limited with `Retry-After`, `503` delivery not configured, `405` on every method but `POST`). **Not verified**: an actual Resend delivery — no real `RESEND_API_KEY` was available during this work, so the `resend.emails.send` call itself, and Resend's real error shapes on failure, are unverified beyond what its published SDK types document. Confirm with a real key before treating this as fully proven end to end.
